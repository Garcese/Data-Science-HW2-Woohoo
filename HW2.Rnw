...
\begin{enumerate}
  ...
  n.10.1 <- rbinom(10, 40, 0.5)
  n.10.2 <- rbinom(10, 20, 0.25)
  n.25.1 <- rbinom(25, 40, 0.5)
  n.25.2 <- rbinom(25, 20, 0.25)
  n.100.1 <- rbinom(100, 40, 0.5)
  n.100.2 <- rbinom(100, 20, 0.25)
  n.1000.1 <- rbinom(1000, 40, 0.5)
  n.1000.2 <- rbinom(1000, 20, 0.25)
  
  stat1 <- data.frame(n.10.1,n.10.2,n.25.1,n.25.2 ,n.100.1,n.100.2,n.1000.1,n.1000.2)
  apply(stat1, 2, mean)
  apply(stat1, 2, sd)
  ...
  binom.mom <- function(par, data){
    ...
    c(eq1, eq2)
    #print(c(EX1, EX2, mean(data), mean(data^2), eq1, eq2))
  ...
  mles <- optim(fn = binom.MLE,
   ...
   print(c(moms))
    ...
    plot_mom = ggplot() +
      geom_histogram(aes(x = rbinom(n, size = moms$x[1], prob = moms$x[2]),
                         y = ..density..
                          ), binwidth = 1, color = "green", fill ="grey") +
      geom_hline(yintercept = 0) +
      theme_bw() + 
    geom_linerange( aes(x=0:40, ymin=0, ymax=dbinom(0:40, size = par[1],
                                               prob = par[2])))
              
        plot_mle = ggplot() +
      geom_histogram(aes(x = rbinom(n, size = par[1], prob = par[2]),
                         y = ..density..), , color = "green", fill ="grey",
                     binwidth = 1) +
      geom_hline(yintercept = 0) +
      theme_bw() + geom_linerange( aes(x=0:40, ymin=0, ymax=dbinom(0:40, size = par[1],
                                               prob = par[2])))
